...
namespace ESFA.DC.ILR1819.ReportService.Service.Mapper
{
    public sealed class FundingSummaryMapper : ClassMap<FundingSummaryModel>, IClassMapper
    {
        public FundingSummaryMapper()
        {
            ...
            Map(m => m.Period1_8).Index(13).Name("Aug-Mar").TypeConverterOption.Format("0.00");
            Map(m => m.Period9_12).Index(14).Name("Apr-Jul").TypeConverterOption.Format("0.00");
            Map(m => m.YearToDate).Index(15).Name("Year To Date").TypeConverterOption.Format("0.00");
            Map(m => m.Total).Index(16).Name("Total").TypeConverterOption.Format("0.00");
        }
    }
}
