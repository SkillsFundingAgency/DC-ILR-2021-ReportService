...
namespace ESFA.DC.ILR.ReportService.Reports.Tests.Funding.DevolvedFunding.Model
{
    public class FundLineGroupTests
    {
        ...
        private DevolvedAdultEducationFundLineGroup NewGroup(string title = "Title", int currentPeriod = 12, FundingDataSources fundModel = FundingDataSources.FM35, IEnumerable<string> fundLines = null, IPeriodisedValuesLookup periodisedValues = null)
        {
            return new DevolvedAdultEducationFundLineGroup(title, currentPeriod, fundModel, fundLines ?? new List<string>(), periodisedValues ?? new PeriodisedValuesLookup());
        }
    }
}
