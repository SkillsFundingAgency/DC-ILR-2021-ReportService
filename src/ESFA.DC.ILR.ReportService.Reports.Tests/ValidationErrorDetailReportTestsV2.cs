...
{
    public class ValidationErrorDetailReportTestsV2
    {
        [Fact]
        public async Task GenerateAsync()
        {
            ...
            fileNameServiceMock.Setup(s => s.GetFilename(reportServiceContext.Object, "Rule Violation Report V2", OutputTypes.Csv, true)).Returns(fileName);
            ...
            csvServiceMock.Verify(s => s.WriteAsync<ValidationErrorRow, ValidationErrorMapperV2>(validationErrorRows, fileName, container, cancellationToken, null, null));
        }

        [Fact]
        public void FileName()
        {
            NewReport().ReportName.Should().Be("Rule Violation Report V2");
        }
        ...
        {
            return new ValidationErrorsDetailReportV2(validationErrorsReportBuilder, csvService, frontEndValidationReport, fileNameService);
        }
    }
}
