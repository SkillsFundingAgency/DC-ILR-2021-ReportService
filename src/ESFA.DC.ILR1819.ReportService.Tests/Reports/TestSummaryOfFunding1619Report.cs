...
namespace ESFA.DC.ILR1819.ReportService.Tests.Reports
{
    public sealed class TestSummaryOfFunding1619Report
    {
        ...
        public async Task TestSummaryOfFunding1619ReportGeneration()
        {
            ...
            storage.Setup(x => x.SaveAsync("16-19 Summary of Funding by Student Report.csv", It.IsAny<string>())).Callback<string, string>((key, value) => csv = value).Returns(Task.CompletedTask);
            ...
            TestHelper.CheckCsv(csv, new SummaryOfFunding1619Mapper());
        }
    }
}
