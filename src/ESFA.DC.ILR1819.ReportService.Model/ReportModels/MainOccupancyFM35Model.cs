namespace ESFA.DC.ILR1819.ReportService.Model.ReportModels
{
    public sealed class MainOccupancyFM35Model
    {
        public string LearnRefNumber { get; set; }

        public long Uln { get; set; }

        public string DateOfBirth { get; set; }

        public string PostcodePrior { get; set; }

        public int? PmUkprn { get; set; }

        public string CampId { get; set; }

        public string ProvSpecLearnMonA { get; set; }

        public string ProvSpecLearnMonB { get; set; }

        public int AimSeqNumber { get; set; }

        public string LearnAimRef { get; set; }

        public string LearnAimRefTitle { get; set; }

        public string SwSupAimId { get; set; }

        public string WeightedRateFromEsol { get; set; }

        public string ApplicWeightFundRate { get; set; }

        public string ApplicProgWeightFact { get; set; }

        public string AimValue { get; set; }

        public string NotionalNvqLevelV2 { get; set; }

        public decimal? SectorSubjectAreaTier2 { get; set; }

        public int? ProgType { get; set; }

        public int? FworkCode { get; set; }

        public int? PwayCode { get; set; }

        public int AimType { get; set; }

        public int? FrameworkComponentType { get; set; }

        public int FundModel { get; set; }

        public int? PriorLearnFundAdj { get; set; }

        public int? OtherFundAdj { get; set; }

        public System.DateTime? OrigLearnStartDate { get; set; }

        public System.DateTime LearnStartDate { get; set; }

        public System.DateTime LearnPlanEndDate { get; set; }

        public int CompStatus { get; set; }

        public System.DateTime? LearnActEndDate { get; set; }

        public int? Outcome { get; set; }

        public System.DateTime? AchDate { get; set; }

        public int? AddHours { get; set; }

        public string LearnDelFamCodeSof { get; set; }

        public string LearnDelFamCodeFfi { get; set; }

        public string LearnDelFamCodeEef { get; set; }

        public string LearnDelFamCodeLsfHighest { get; set; }

        public string LearnDelFamCodeLsfEarliest { get; set; }

        public string LearnDelFamCodeLsfLatest { get; set; }

        public string LearnDelFamCodeLdm1 { get; set; }

        public string LearnDelFamCodeLdm2 { get; set; }

        public string LearnDelFamCodeLdm3 { get; set; }

        public string LearnDelFamCodeLdm4 { get; set; }

        public string LearnDelFamCodeRes { get; set; }

        public string ProvSpecDelMonA { get; set; }

        public string ProvSpecDelMonB { get; set; }

        public string ProvSpecDelMonC { get; set; }

        public string ProvSpecDelMonD { get; set; }

        public string FundLine { get; set; }

        public int? PlannedNumOnProgInstalm { get; set; }

        public int? PlannedNumOnProgInstalmTrans { get; set; }

        public string StartPropTrans { get; set; }

        public string AchieveElement { get; set; }

        public decimal AchievePercentage { get; set; }

        public string NonGovCont { get; set; }

        public int? PartnerUkprn { get; set; }

        public string DelLocPostCode { get; set; }

        public decimal? AreaCostFactAdj { get; set; }

        public string DisUpFactAdj { get; set; }

        public string LargeEmployerID { get; set; }

        public string LargeEmployerFM35Fctr { get; set; }

        public string CapFactor { get; set; }

        public string TraineeWorkPlacement { get; set; }

        public string HigherApprentishipHeAim { get; set; }

        public object ApplicEmpFactDate { get; set; }

        public System.DateTime? ApplicFactDate { get; set; }

        public decimal? Period1OnProgPayment { get; set; }

        public decimal? Period1BalancePayment { get; set; }

        public decimal? Period1AchievePayment { get; set; }

        public decimal? Period1EmpOutcomePay { get; set; }

        public decimal? Period1LearnSuppFundCash { get; set; }

        public decimal? Period2OnProgPayment { get; set; }

        public decimal? Period2BalancePayment { get; set; }

        public decimal? Period2AchievePayment { get; set; }

        public decimal? Period2EmpOutcomePay { get; set; }

        public decimal? Period2LearnSuppFundCash { get; set; }

        public decimal? Period3OnProgPayment { get; set; }

        public decimal? Period3BalancePayment { get; set; }

        public decimal? Period3AchievePayment { get; set; }

        public decimal? Period3EmpOutcomePay { get; set; }

        public decimal? Period3LearnSuppFundCash { get; set; }

        public decimal? Period4OnProgPayment { get; set; }

        public decimal? Period4BalancePayment { get; set; }

        public decimal? Period4AchievePayment { get; set; }

        public decimal? Period4EmpOutcomePay { get; set; }

        public decimal? Period4LearnSuppFundCash { get; set; }

        public decimal? Period5OnProgPayment { get; set; }

        public decimal? Period5BalancePayment { get; set; }

        public decimal? Period5AchievePayment { get; set; }

        public decimal? Period5EmpOutcomePay { get; set; }

        public decimal? Period5LearnSuppFundCash { get; set; }

        public decimal? Period6OnProgPayment { get; set; }

        public decimal? Period6BalancePayment { get; set; }

        public decimal? Period6AchievePayment { get; set; }

        public decimal? Period6EmpOutcomePay { get; set; }

        public decimal? Period6LearnSuppFundCash { get; set; }

        public decimal? Period7OnProgPayment { get; set; }

        public decimal? Period7BalancePayment { get; set; }

        public decimal? Period7AchievePayment { get; set; }

        public decimal? Period7EmpOutcomePay { get; set; }

        public decimal? Period7LearnSuppFundCash { get; set; }

        public decimal? Period8OnProgPayment { get; set; }

        public decimal? Period8BalancePayment { get; set; }

        public decimal? Period8AchievePayment { get; set; }

        public decimal? Period8EmpOutcomePay { get; set; }

        public decimal? Period8LearnSuppFundCash { get; set; }

        public decimal? Period9OnProgPayment { get; set; }

        public decimal? Period9BalancePayment { get; set; }

        public decimal? Period9AchievePayment { get; set; }

        public decimal? Period9EmpOutcomePay { get; set; }

        public decimal? Period9LearnSuppFundCash { get; set; }

        public decimal? Period10OnProgPayment { get; set; }

        public decimal? Period10BalancePayment { get; set; }

        public decimal? Period10AchievePayment { get; set; }

        public decimal? Period10EmpOutcomePay { get; set; }

        public decimal? Period10LearnSuppFundCash { get; set; }

        public decimal? Period11OnProgPayment { get; set; }

        public decimal? Period11BalancePayment { get; set; }

        public decimal? Period11AchievePayment { get; set; }

        public decimal? Period11EmpOutcomePay { get; set; }

        public decimal? Period11LearnSuppFundCash { get; set; }

        public decimal? Period12OnProgPayment { get; set; }

        public decimal? Period12BalancePayment { get; set; }

        public decimal? Period12AchievePayment { get; set; }

        public decimal? Period12EmpOutcomePay { get; set; }

        public decimal? Period12LearnSuppFundCash { get; set; }

        public decimal? TotalOnProgPayment { get; set; }

        public decimal? TotalBalancePayment { get; set; }

        public decimal? TotalAchievePayment { get; set; }

        public decimal? TotalEmpOutcomePay { get; set; }

        public decimal? TotalLearnSuppFundCash { get; set; }

        public decimal? TotalEarnedCash { get; set; }

        public string OfficalSensitive { get; }
    }
}
